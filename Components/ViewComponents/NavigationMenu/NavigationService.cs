...
namespace DancingGoat.ViewComponents
{
    public class NavigationService
    {
        ...
        public async Task<IEnumerable<NavigationItemViewModel>> GetNavigationItemViewModels(
            string languageName,
            CancellationToken cancellationToken = default
        )
        {
            var navigationItems = (
                await navigationItemRepository.GetNavigationItems(languageName, cancellationToken)
            ).ToList();
            ...
            var navigationModels = await GetModelsCached(
                navigationItems,
                menuItemGuids,
                languageName,
                cancellationToken
        ...
        private async Task<IEnumerable<NavigationItemViewModel>> GetModelsCached(
            List<NavigationItem> navigationItems,
            List<Guid> menuItemGuids,
            string languageName,
        ...
        )
        {
            var cacheSettings = new CacheSettings(
                ...
                nameof(GetNavigationItemViewModels),
                languageName
            );
                ...
                async (settings, cancellationToken) =>
                {
                    ...
                    var navigationModels = navigationItems
                        .Where(navigationItem =>
                            urls.ContainsKey(navigationItem.NavigationItemLink.First().WebPageGuid)
                        )
                        .Select(navigationItem => new NavigationItemViewModel(
                        ...
                        foreach (var key in menuItemGuids)
                        {
                            cacheKeys.Add(
                                CacheHelper.BuildCacheItemName(
                                    new[] { "webpageitem", "byguid", key.ToString() },
                                    false
                                )
                            );
                        }
                        ...
                        cacheKeys.Add(
                            CacheHelper.BuildCacheItemName(
                                new[]
                                {
                                    ...
                                    websiteChannelContext.WebsiteChannelName,
                                    "childrenofpath",
                                    DancingGoatConstants.NAVIGATION_MENU_FOLDER_PATH,
                                }
                            )
                        );
